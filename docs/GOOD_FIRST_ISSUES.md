...
1. **Accessibility: preview focus order** (`accessibility`, `good first issue`)
   — verify the editable preview with keyboard-only navigation and add a
   regression assertion for focus order.
2. **Renderer: long-title fixture** (`renderer`, `good first issue`) — add one
   approved wrapping fixture and document the viewer result.
3. **Documentation: translated quick start** (`documentation`, `help wanted`)
   — translate the no-key path while preserving privacy boundaries.
4. **Template: nonprofit decision brief** (`templates`, `good first issue`) —
