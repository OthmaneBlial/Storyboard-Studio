...
roadmap. `make test` : 184 tests Python, sans avertissement de dépréciation
...
sur 184 tests (voir [`docs/COVERAGE.md`](docs/COVERAGE.md)). La parité des
...
Une voie native expérimentale a ensuite été vérifiée localement sur macOS
ARM64 : le binaire PyInstaller de 37 Mo exécute `--version` et génère un vrai
PPTX de démonstration sans dépendre d'un interpréteur Python externe dans le
`PATH` lorsque les données de `storyboard_studio` et les templates `python-pptx`
sont embarqués. La
